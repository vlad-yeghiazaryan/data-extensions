...
class EmptySeriesImputer():
    ...
    """
    def __init__(self, fill_method, scaler=None, recursive_imputation=True,
    ...
    def fit_model(self, X, y):
        coef, _, rank, singular = linalg.lstsq(X, y)
        return coef
    ...
    def aggregate_estimation(self, df_groups, group_index, variable_index):
        # setup exog
        X = df_groups[group_index]
        finite_col_indices = np.where(np.isfinite(X).all(axis=0))[0]
        X = X[:, finite_col_indices]

        # estimate coefficients from all groups
        fitted_coefs = []
        for i in range(df_groups.shape[0]):
            group_item = df_groups[i]
            if (i!=group_index) and (group_item.size!=0):
                ...
                if (finite_col_indices_group.size != 0) and not np.isnan(y_group.sum()):
                    ...
                    group_coefs[finite_col_indices_group] = some_group_coefs
                    fitted_coefs.append(group_coefs)
        fitted_coefs = np.array(fitted_coefs)
        fitted_preds = X @ fitted_coefs.T
        return fitted_preds

    def recursive_impute(self, df_scaled, df_filled, missing_priority):
        ...
        df_groups = np.array(np.split(df_array, len(group_names)))
        #  perform (recursive) imputation
        for group, variable in missing_priority:
            group_index = np.where(group_names == group)[0][0]
            variable_index = np.where(columns == variable)[0][0]
    ...
    def impute(self, df):
        ...
        #  perform (recursive) imputation
        df_filled = self.recursive_impute(df_scaled, df_filled, self.missing_priority)
        return self.package_df(df_filled, Flipped)
    ...
    def make_replica(self, df, original_missing_indices=None):
        ...
        # Standardize and add constant
        df_scaled = df.groupby(self.fill_method, group_keys=False).apply(self.standardize_series)
        df_replica = df_scaled.copy()
        df_scaled['const'] = 1

        # defining partial replica missing indices
        ...
        # defining search space and iterating over all columns
        matrix_map = df.groupby(self.fill_method).sum().stack().index
        for item in matrix_map:
            group, variable = item
            gi = np.where(group_names == group)[0][0]
            vi = np.where(columns == variable)[0][0]
            df_groups[gi, :, vi] = np.NaN

            #  perform imputation
            fitted_preds = self.aggregate_estimation(df_groups, gi, vi)
            replica_values = self.aggregate_predictions(fitted_preds)
            if not (item in original_missing_indices):
                if self.recursive_replication:
                    df_groups[gi, :, vi] = replica_values
                else:
                    df_groups[gi, :, vi] = df_replica.loc[item].values
